...
namespace Game
{
    ...
    public class EnemyControler : MonoBehaviour, IEnemy
    {
        ...
        private Vector3 _moveDir; // 로컬 이동방향
        private Vector3 _worldMoveDir; // 글로벌 이동방향
        ...
        private IEnumerator PatrolMove()
        {
            while(true)
            {
                _moveDir = FindNewPoint(); // 이동 방향 지정
                CalculateMoveDir();
                yield return StartCoroutine(ApplyRotateCoroutine()); // 이동 방향으로 선회가 끝날때까지 대기 후 이동
                while (!MoveSet.IsUnableMove())
                    yield return _waitFixedUpdate; // 이후 이동이 불가능할때까지 지정된 이동방향으로 직진
        ...
        private IEnumerator ApplyRotateCoroutine()
        {        
            float prevY = ShareCom.enemyRoot.localEulerAngles.y;  //현재 방향
            float nextY = Quaternion.LookRotation(_worldMoveDir).eulerAngles.y; //지정된 방향
            ...
            //     roteProcess += _deltaTime;
                ShareCom.enemyRoot.localEulerAngles = Vector3.up * Mathf.Lerp(prevY, nextY, roteProcess / Stats.patrolRotateTime);
                yield return _waitFrame;
            } // Stats.patrolRotateTime 에 걸쳐 현재 방향에서 지정된 방향으로 선회시킴
            ShareCom.enemyRoot.localEulerAngles = Vector3.up * nextY; 
            State.isMoving = true; // 방향지정이 끝나면 이동을 시작함
        }
        ...
        private void EnemyMove()
        {
            ...
            CalculateMoveDir();
            ApplyRotate();
            MoveSet.SetMovement(_worldMoveDir);
        }

        private void ApplyRotate()
        {
            float currentY = ShareCom.enemyRoot.localEulerAngles.y;
            float nextY = Quaternion.LookRotation(_worldMoveDir).eulerAngles.y;

            if (nextY - currentY > 180f) nextY -= 360;
            else if (currentY - nextY > 180f) nextY += 360;

            ShareCom.enemyRoot.localEulerAngles = Vector3.up * Mathf.Lerp(currentY, nextY, 0.3f);
        }
    }
}